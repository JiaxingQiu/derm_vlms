#!/usr/bin/env python
"""DermatoLlama reason-based differential prediction on all MIDAS lesions."""
...
sys.path.insert(0, os.path.join(PROJECT_ROOT, "collect_ai_response", "dermato_llama"))
...
OUT_PATH = os.path.join(RESULTS_DIR, "dermato_llama_predictions_reason.csv")
...
COL_ORDER = [
    ...
    "reason_classify", "image_path", "original_image_name", "lesion_id",
]
...
def main():
    ...
    for _, row in tqdm(pending.iterrows(), total=len(pending)):
        ...
        batch.append({
            ...
            "reason_classify": predict_image(model, processor, image, prompt=PROMPT),
