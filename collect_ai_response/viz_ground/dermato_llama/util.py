"""Self-grounding utilities for DermatoLlama.

Provides the same interface as viz_ground/qwen3_8b/util.py but uses
DermatoLlama (Llama-3.2-11B-Vision + LoRA) to draw bounding boxes for its
OWN reasoning sentences — so the box and reasoning are coherent from one model.

Note: DermatoLlama was NOT trained for grounding, so parse success rates
may be lower than GPT-5.3 or even MedGemma.
"""
...
def load_model(
    ...
    """Load DermatoLlama (base + LoRA adapter). Returns (model, processor)."""
    kwargs = {}
    if hf_token:
    ...
    processor = AutoProcessor.from_pretrained(base_model_id, **kwargs)

    model = PeftModel.from_pretrained(model, adapter_path, **kwargs)
    model.eval()
    n_params = sum(p.numel() for p in model.parameters())
    print(f"Loaded {base_model_id} + {adapter_path}  ({n_params:,} params)")
...
def _build_inputs(processor, image: Image.Image, messages: list[dict]):
...
def predict_grounding_box(
    ...
    """Ask DermatoLlama to locate the image region for *sentence*.
